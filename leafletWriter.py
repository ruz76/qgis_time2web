...
def writeLeaflet(iface, outputProjectFileName, layer_list, visible, cluster,
                 json, params, popup, timefrom, timeto):
    legends = {}
    ...
    timemin = params["Time axis"]["Min"]
    timemax = params["Time axis"]["Max"]
    ...
    #print """AAA""" + str(timemin)    
    middle = """
        <p style=\"position: fixed; top: 0; right: 0;\">Time axis: <input type=\"range\" id=\"date\" min=\"{timemin}\" max=\"{timemax}\"/><input id=\"datetxt\"/></p>""".format(timemin=str(timemin), timemax=str(timemax))
    middle += openScript()
        ...
        if i.type() == QgsMapLayer.VectorLayer:
            (new_src,
             ...
             wfsLayers) = writeVectorLayer(i, safeLayerName, usedFields,
                                           ...
                                           visible, json, legends, new_src, timefrom, timeto)
            visibility_src += """
            setVisibility""" + safeLayerName + """();""" 
    ...
    new_src += """
        ...
        feature_group.addTo(map);"""
    new_src += """
        function setVisibility() {""" +  visibility_src + """};"""
