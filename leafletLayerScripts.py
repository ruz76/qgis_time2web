...
def writeVectorLayer(i, safeLayerName, usedFields, highlight, popupsOnHover,
                     ...
                     cluster_num, visible, json, legends, new_src, timefrom, timeto):
    (new_pop, labeltext,
    ...
    if (isinstance(renderer, QgsSingleSymbolRendererV2) or
        ...
        (new_obj, legends,
         wfsLayers) = singleLayer(renderer, outputProjectFileName,
                                  ...
                                  json, usedFields, legends, count, popFuncs, timefrom, timeto)
...
""" + new_obj
	...
	#ruzicka
	#TODO
    #Vyresit problem s omezenim na max 1000 objektu ve vrstve
	new_src += """
		function setVisibility""" + safeLayerName + """() {   
		 for (var row=0; row<1000; row++) {
		  if ( typeof(json_""" + safeLayerName + """JSON._layers[row])=='undefined') continue;
		  s = doStyle""" + safeLayerName + """(json_""" + safeLayerName + """JSON._layers[row].feature);
		  json_""" + safeLayerName + """JSON._layers[row].setStyle(s);
		 }      
		};"""
...
def singleLayer(renderer, outputProjectFileName, safeLayerName, wfsLayers, i,
                ...
                usedFields, legends, count, popFuncs, timefrom, timeto):
    ...
    if i.geometryType() == QGis.Point:
        ...
        new_obj, wfsLayers = singlePolygon(i, safeLayerName, symbol,
                                           ...
                                           usedFields, wfsLayers, count, timefrom, timeto)
...
def singlePoint(symbol, symbolLayer, layer_transp, symbol_transp,
                ...
                outputProjectFileName, timefrom, timeto):
    ...
    if isinstance(symbolLayer, QgsSvgMarkerSymbolLayerV2):
        ...
        pointStyleLabel = pointStyleLabelScript(safeLayerName, radius,
                                                ...
                                                labeltext, timefrom[count], timeto[count])
...
def singleLine(symbol, colorName, fill_opacity, i, json, safeLayerName,
               wfsLayers, visible, usedFields, count, timefrom, timeto):
    radius = symbol.width()
    ...
    lineStyle = simpleLineStyleScript(radius, colorName, penStyle, capString,
                                      joinString, fill_opacity, timefrom[count], timeto[count])
    if i.providerType() == 'WFS' and json[count] is False:
